...
suspend fun doInvokeMethod(
    ...
    // Special case: if the method expects a List<Any?> as the only parameter (besides the receiver),
    // pass the args list as-is instead of adding actor to the front
    if (parameterTypes.size == 2 &&
        parameterTypes[0].kind == KParameter.Kind.INSTANCE &&
        parameterTypes[1].type.jvmErasure == List::class) {
        // Method signature is like: fun methodName(args: List<Any?>)
        processedArgs.add(actor)  // Add the receiver (actor)
        processedArgs.add(args)   // Add the args list as the single parameter
    } else {
        // Original logic: add actor to the front of args
